...
def save_npz(train_data):
		global maxlen, num_words
...
for sequence in sequences_data:
		if len(sequence) < maxlen:
				...
				if len(sequence) > 0:
						structured_data.append(sequence)
train_data = pad_sequences(structured_data, maxlen, padding="pre")  #all the sequences converted to the same length, the maxlen
